...
class SparselyUpdatedStateDStream[V: ClassTag, S: ClassTag](
  ...
  override def compute(validTime: Time): Option[IndexedRDD[S]] = {
      ...
      case Some(prevStateRDD) =>    // If previous state RDD exists
        ...
        parent.getOrCompute(validTime) match {
          case Some(parentRDD) =>   // If parent RDD exists, then compute as usual
            ...
            val stateRDD = prevStateRDD.asInstanceOf[IndexedRDD[S]].multiputRDDWithDeletion[Iterable[V]](
              updatesByKey,
          ...
          case None =>    // If parent RDD does not exist
            ...
            Some(prevStateRDD.asInstanceOf[IndexedRDD[S]])
        }
      ...
      case None =>    // If previous session RDD does not exist (first input data)
        ...
        parent.getOrCompute(validTime) match {
          case Some(parentRDD) =>   // If parent RDD exists, then compute as usual
            ...
            val updatesByKey = parentRDD.groupByKey(partitioner)
            val updateFuncLocal = updateFunc
            val stateRDD = IndexedRDD(updatesByKey.flatMapValues(
