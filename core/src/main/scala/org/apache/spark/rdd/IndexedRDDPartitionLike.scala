...
private[spark] trait IndexedRDDPartitionLike[V, Self[X] <: IndexedRDDPartitionLike[X, Self]]
  ...
  /**
   * Updates the keys in `kvs` to their corresponding values generated by running `insert` if there
   * is no existing key, or by running `merge` if there is one. Returns a new IndexedRDDPartition
   * that reflects the modification.
   */
  def multiput[U: ClassTag](
      kvs: Seq[(Id, U)], insert: (Id, U) => V, merge: (Id, V, U) => V): Self[V] = {
    if (kvs.forall(kv => self.isDefined(kv._1))) {
      // Pure updates can be implemented by modifying only the values
      join(kvs.iterator)(merge)
    } else {
      multiputIterator(kvs.iterator, insert, merge)
    }
  }

  private def multiputIterator(kvs: Iterator[Product2[Id, V]], merge: (Id, V, V) => V): Self[V] = {
    multiputIterator[V](kvs, (id, v) => v, merge)
  }

  private def multiputIterator[U: ClassTag](
      kvs: Iterator[Product2[Id, U]], insert: (Id, U) => V, merge: (Id, V, U) => V): Self[V] = {
