...
private[spark] trait IndexedRDDLike[
  ...
  def multiput(kvs: Map[Id, V], merge: (Id, V, V) => V): Self[V] = {
    ...
    multiput[V](updates, (id, v) => v, merge)
  }
   ...
   * that reflects the modification.
   */
  def multiput[U: ClassTag](kvs: RDD[(Id, U)], insert: (Id, U) => V,
      merge: (Id, V, U) => V): Self[V] = {
    val updates = kvs.partitionBy(self.partitioner.get)
    zipPartitionsWithOther(updates)(new MultiputZipper(insert, merge))
  }
  ...
  private class MultiputZipper[U: ClassTag](insert: (Id, U) => V, merge: (Id, V, U) => V)
    ...
    def apply(thisIter: Iterator[P[V]], otherIter: Iterator[(Id, U)]): Iterator[P[V]] = {
      ...
      val updates = otherIter.toArray
      Iterator(thisPart.multiput(updates, insert, merge))
    }
  }
