...
private[spark] trait IndexedRDD[
   ...
   */
  def multiput(kvs: Map[Id, V]): Self[V] = multiput(kvs, (id: Id, a: V, b: V) => b)
  ...
  def multiput(kvs: Map[Id, V], merge: (Id, V, V) => V): Self[V] = {
    multiput(self.context.parallelize(kvs.toSeq), merge)
  }

  /**
   * Updates the keys in the RDD `kvs` to their corresponding values, running `merge` on old and new
   * values if necessary. Returns a new IndexedRDD that reflects the modification.
   */
  def multiput(kvs: RDD[(Id, V)], merge: (Id, V, V) => V): Self[V] = {
    val updates = kvs.partitionBy(self.partitioner.get)
    zipPartitionsWithOther(updates)(new MultiputZipper1(merge))
  }

  /**
   * Updates the keys in the RDD `kvs` to their corresponding values generated by running `insert`
   * if there is no existing key, or by running `merge` if there is one. Returns a new IndexedRDD
   * that reflects the modification.
   */
  def multiput[U: ClassTag](
      kvs: RDD[(Id, U)], insert: (Id, U) => V, merge: (Id, V, U) => V): Self[V] = {
    val updates = kvs.partitionBy(self.partitioner.get)
    zipPartitionsWithOther(updates)(new MultiputZipper2(insert, merge))
  }

  def multiputWithDeletion[U: ClassTag](
      kvs: RDD[(Id, U)], insert: (Id, U) => Option[V], merge: (Id, V, U) => Option[V]): Self[V] = {
    val updates = kvs.partitionBy(self.partitioner.get)
    zipPartitionsWithOther(updates)(new MultiputWithDeletionZipper(insert, merge))
  }
  ...
  private class MultiputZipper1(merge: (Id, V, V) => V)
    ...
    def apply(thisIter: Iterator[P[V]], otherIter: Iterator[(Id, V)]): Iterator[P[V]] = {
      ...
      Iterator(thisPart.multiput(otherIter, merge))
    }
  }

  private class MultiputZipper2[U: ClassTag](insert: (Id, U) => V, merge: (Id, V, U) => V)
    ...
    def apply(thisIter: Iterator[P[V]], otherIter: Iterator[(Id, U)]): Iterator[P[V]] = {
      val thisPart = thisIter.next()
      Iterator(thisPart.multiput(otherIter, insert, merge))
    }
  }

  private class MultiputWithDeletionZipper[U: ClassTag](
