...
class NonUpdatableIndexedRDDSuite extends FunSuite with SharedSparkContext {

  def pairs(sc: SparkContext, n: Int) = {
    NonUpdatableIndexedRDD(sc.parallelize((0 to n).map(x => (x.toLong, x)), 5))
  }
  ...
  test("leftJoin") {
    ...
    val ps = pairs(sc, n).cache()
    val evens = ps.filter(q => ((q._2 % 2) == 0)).cache()
    // leftJoin with another NonUpdatableIndexedRDD
  ...
  test("join") {
    ...
    val ps = pairs(sc, n).cache()
    val evens = ps.filter(q => ((q._2 % 2) == 0)).cache()
    // join with another NonUpdatableIndexedRDD
  ...
  test("innerJoin") {
    ...
    val ps = pairs(sc, n).cache()
    val evens = ps.filter(q => ((q._2 % 2) == 0)).cache()
    // innerJoin with another NonUpdatableIndexedRDD
