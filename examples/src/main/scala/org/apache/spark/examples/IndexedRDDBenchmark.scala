...
object IndexedRDDBenchmark {
  ...
  def test[
    ...
    time("multiputRDD - subset", trials) {
      indexed.multiputRDD(subsetRDD, (id, a, b) => a + b).count()
    }
    time("multiputRDD - no subset", trials) {
      indexed.multiputRDD(incomparableRDD, (id, a, b) => a + b).count()
    }
